...
/* image_set -- switch off a single pixel in an image */
void image_unset(int x, int y, unsigned *img)
{
    if (x < 0 || x >= 5 || y < 0 || y >= 5)
        return;
    unsigned p = map_pixel(x, y);
...
/* state_pixel -- get the current state of the pixel in an image 0 -> off 1 -> on*/
int state_pixel(int x,int y, const unsigned *img){
    if (x < 0 || x >= 5 || y < 0 || y >= 5)
        return - 1;
    unsigned p = map_pixel(x, y);
    return GET_BIT(img[XPART(p)], YPART(p));
}
